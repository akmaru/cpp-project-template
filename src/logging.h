...
#define CHECK(cond) \
  while (!(cond)) cpp_template::FailMessageStream("Check `" #cond "' failed!", __func__, __FILE__, __LINE__)

#define CHECK_CMP(a, b, op)                                                                                \
  while (!((a)op(b)))                                                                                      \
  cpp_template::FailMessageStream("Check `" #a "' " #op " `" #b "' failed!", __func__, __FILE__, __LINE__) \
      << "(" << (a) << " vs " << (b) << ") "
